...
| `CANVAS_SDK.md` | 0.6 | Canvas registration owned by IH (§3 is a pointer, not a schema). §2.6 enumerates first-party worker-shell views (Marketplace, Earnings, Vetting, Settings) vs canvas-owned right pane (PRD alignment). §4.5 MCP-shaped runtime capability discovery. §5 two credential types (backend client creds + per-worker session JWT). `canvas_id` platform-assigned. |
